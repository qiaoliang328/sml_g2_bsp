...
dd if=/dev/zero of=$device bs=512 count=5120 seek=0
...
# make partitions.
echo "Formatting ${device} ${PARTITION1} ..."
for i in { 1..5 }
do
  if [ -b ${PARTITION1} ]; then
    break
...
if [ ! -b ${PARTITION1} ]; then
  ...
  if [ ${label} != "SD" ]; then
    ...
    mkfs.fat -F 32 -s 128 ${PARTITION1}
  fi
fi
if [ $? -ne 0 ]; then
  echo "mkfs.fat '$device' failed"
  exit 1
fi
exit 0
